...
namespace Plugin.BLE.Android
{
    public class Adapter : AdapterBase
    {
        ...
        private readonly BluetoothLeAdvertiser _bluetoothLeAdvertiser;
...
#pragma warning restore 618
        ...
        public override void StartAdvertising(string id)
        {
            var builder = new AdvertiseSettings.Builder();
            builder.SetAdvertiseMode(AdvertiseMode.LowPower);
            builder.SetTimeout(18000);
            var settings = builder.Build();

            var dataBuilder = new AdvertiseData.Builder();
            byte[] bytes = Encoding.ASCII.GetBytes("attica");
            dataBuilder.AddServiceData(ParcelUuid.FromString("8C443036-B927-4965-96DE-83F39C1ABA5A"), bytes);
            dataBuilder.SetIncludeDeviceName(false);
            dataBuilder.SetIncludeTxPowerLevel(false);

            var data = dataBuilder.Build();

            var callback = new AdvertiseCallbackImp();


            _bluetoothLeAdvertiser.StartAdvertising(settings, data, callback);
        }
